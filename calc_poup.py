...
def buscar_rend():
    ...
    label_total['text'] = ''
    lista = open('D:/pys/pycharm/calcpoup/rendimento.txt', 'r')
    l_rend = []
    for i in lista:
        l_rend.append(i[:-1])
    lista.close()
...
button_calcular = Button(screen, text='Calcular valor', command=valor_total, padx=padvar, pady=padvar, font=fontsize)
...
button_data.grid(column=2, row=2, sticky='we')
label_estado = Label(screen, text='', padx=padvar, pady=padvar, relief='sunken', font=fontsize)
label_estado.grid(column=2, rowspan=4, sticky='nsew')
...
label_valor_rend.grid(column=1, row=4, sticky='we')
...
label_total.grid(column=1, row=5, sticky='we')
